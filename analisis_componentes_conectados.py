...
def main():
    # Comprobar si hay una diapositiva cargada en el estado de la sesión
    if 'uploaded_slide' in st.session_state:
        ...
        area_threshold_slider = st.slider('Valor mínimo de área a considerar', min_value=1, max_value=2000)
        ...
        for i in range(1, totalLabels):
            ...
            if area > area_threshold_slider:  # Filtrar por tamaño mínimo de área
                ...
                # Crear una nueva imagen para mostrar el componente individual
                component = np.zeros(gray.shape, dtype="uint8")
                componentMask = (label_ids == i).astype("uint8") * 255

                # Aplicar la máscara utilizando el operador bitwise
        ...
        if component_data:
            ...
            st.write(f"### Porcentaje total de área cubierta: {total_percentage:.2f}%")
        ...
        st.markdown("---")
        st.header("Por favor, selecciona una imagen para poder continuar")
        st.markdown("---")
